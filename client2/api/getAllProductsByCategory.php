<?php
  ...
  if($q == ""){
    $sql = $conn -> query("SELECT * FROM product WHERE category='$category' AND status='active'");
  }else {
    $sql = $conn -> query("SELECT * FROM product WHERE name LIKE '%$q%' AND category='$category' AND status='active'");
  }
      ...
      echo "
        <div onclick='quickView(this.id)' class='is-flex is-flex-direction-column card p-4' id='".$row['id']."'>
          ...
          <span class='ml-auto me-2 is-size-7' style='color: #000 !important;'>".$row['sold']." sold </span>
            <span class='is-size-7' style='color: #000 !important;'>".$row['stock']." stock </span>
        </div>
        </div>
      ";
    }
